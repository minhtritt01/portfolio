...
import { X, Mail, MessageCircle, Contact } from 'lucide-react'
...
/** Nudges visitors who have stuck around for 15s. Once per session. */
export default function ContactPopup({ lang }: { lang: Lang }) {
  ...
  const handleCtaClick = (cta: string) => trackEvent('popup_cta_click', { cta })
      ...
      {open && (
        <>
          ...
          >
            ...
            <h3 className="font-display text-lg font-bold text-foreground pr-6 mb-2">{t.popup.heading}</h3>
            <p className="text-sm text-muted-foreground leading-relaxed mb-4">{t.popup.body}</p>
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                <Mail className="w-4 h-4" /> {t.popup.emailCta}
              </a>
              <a
              ...
              >
                <LinkedInIcon /> {t.popup.linkedinCta}
              </a>
              <div className="flex gap-2">
                ...
                >
                  <WhatsAppIcon /> {t.popup.whatsappCta}
                </a>
                <a
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-lg text-sm font-medium bg-muted/60 hover:bg-muted border border-border text-foreground/80 transition-all duration-200"
                >
                  <MessageCircle className="w-4 h-4" /> {t.popup.zaloCta}
                </a>
              </div>
              <a
                ...
                className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-medium bg-muted/60 hover:bg-muted border border-border text-foreground/80 transition-all duration-200"
              >
                <Contact className="w-4 h-4" /> {t.popup.cvCta}
              </a>
            </div>
          </motion.div>
        </>
