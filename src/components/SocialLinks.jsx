    ...
    {links.map(({ id, icon, label, href, download }) => (
      <div key={id} className="relative group/link">
        <a
          ...
          download={download}
          aria-label={label}
          className="w-9 h-9 rounded-full flex items-center justify-center bg-card/90 border border-border text-muted-foreground hover:text-primary hover:border-primary/60 hover:bg-muted backdrop-blur-sm transition-all duration-200 shadow-lg"
        >
          {icon}
        </a>
        <span className="pointer-events-none absolute left-11 top-1/2 -translate-y-1/2 bg-card border border-border text-foreground/90 text-xs px-2 py-1 rounded-md opacity-0 group-hover/link:opacity-100 transition-opacity duration-200 whitespace-nowrap shadow-xl">
          {label}
        </span>
      </div>
    ))}
