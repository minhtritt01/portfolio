...
function HomeToc({ lang }: { lang: Lang }) {
  ...
  const tocNav = (
    <nav aria-label="Table of contents" className="relative">
      ...
      <div
        className="absolute left-[5.5px] top-[14px] w-px bg-border"
            ...
            <li key={section.id} className="flex items-center gap-3">
              <motion.span
                className={`relative z-10 w-3 h-3 rounded-full border-2 shrink-0 transition-colors duration-300 ${
                  isActive
                    ? "border-primary bg-primary shadow-[0_0_8px_rgba(var(--primary-rgb),0.4)]"
                    : isPast
      ...
      {visible && (
        <>
          ...
          {tocOpen && (
            <>
              <div
                className="2xl:hidden fixed inset-0 bg-background/60 backdrop-blur-sm z-40"
...
function renderHighlightedText(
  ...
  if (
    typewriterRanges.length === 0 &&
    finalRanges.length === 0 &&
    permanentRanges.length === 0
  ) {
    return (
      <span
        ...
        parts.push(
          <span key={`${baseKey}w${wIdx}`} className="inline-grid">
            <span
              ...
              style={{
                backgroundImage:
                  "linear-gradient(to right, hsl(var(--gradient-from)), hsl(var(--gradient-to)))",
            ...
            <span
              className={`col-start-1 row-start-1 text-muted-foreground transition-opacity ${timing} ${normalOpacity}`}
            >
              {word}
            </span>
          </span>,
  ...
  switch (action.type) {
    ...
    case "DELETE_WORD": {
      ...
      return {
        ...state,
        displayText:
          lastSpace === -1 ? "" : state.displayText.slice(0, lastSpace + 1),
      };
    }
    case "NEXT_REFLECTION":
      return {
        ...state,
        currentReflection: state.currentReflection + 1,
        displayText: "",
        phase: "reflection",
      };
    case "COMPLETE_HOOK_LINE": {
      const newCompleted = [...state.completedHookLines];
      if (!newCompleted[state.currentHookParagraph])
        newCompleted[state.currentHookParagraph] = [];
      newCompleted[state.currentHookParagraph][state.currentHookLine] =
        action.text;
    ...
    case "NEXT_HOOK_LINE":
      return {
        ...state,
        currentHookLine: state.currentHookLine + 1,
        displayText: "",
      };
    case "NEXT_HOOK_PARAGRAPH":
      return {
        ...state,
        currentHookParagraph: state.currentHookParagraph + 1,
        currentHookLine: 0,
        displayText: "",
      };
...
function ReflectiveTypewriter({
  ...
  useEffect(() => {
    ...
    if (seen) {
      dispatch({
        type: "SKIP_TO_COMPLETE",
        allHookLines: allHookLinesComplete,
      });
    }
  ...
  useEffect(() => {
    ...
    if (phase === "context") {
      ...
      if (displayText === cleanContext) {
        ...
        const prevChar =
          displayText.length > 0 ? cleanContext[displayText.length - 1] : "";
    ...
    if (phase === "reflection") {
      ...
      if (displayText === currentText) {
        ...
        const prevChar =
          displayText.length > 0 ? currentText[displayText.length - 1] : "";
        const delay = getTypingDelay(nextChar, prevChar);
        const timer = setTimeout(() => {
          if (signal?.aborted) return;
          dispatch({ type: "TICK", char: nextChar });
    ...
    if (phase === "hook") {
      const flatIndex = (() => {
        let idx = 0;
        for (let p = 0; p < currentHookParagraph; p++)
          idx += hookParagraphs[p].length;
      ...
      if (displayText === currentText) {
        ...
        const isLastLine =
          currentHookLine >= hookParagraphs[currentHookParagraph].length - 1;
        const isLastParagraph =
          currentHookParagraph >= hookParagraphs.length - 1;
        ...
        const prevChar =
          nextCharIndex > 0 ? currentText[nextCharIndex - 1] : "";

        const { slowRanges } = parsedHookLines[flatIndex];
        const isInSlowRange = slowRanges.some(
          ([start, end]) => nextCharIndex >= start && nextCharIndex < end,
        );

        const textSoFar = currentText.slice(0, nextCharIndex);
        const isAfterSentenceEnd =
          prevChar === "." && nextChar === " " && textSoFar.includes("negocio");
  ...
  return (
    <div
      ...
      title={
        phase !== "complete" && phase !== "idle" ? "Click to skip" : undefined
      }
    >
      {/* Context line */}
      ...
      {(phase === "hook" || phase === "complete") &&
        hookParagraphs.map((paragraph, pIdx) => {
          const Tag = pIdx === 0 ? "span" : "p";
          const wrapperClass = pIdx === 0 ? "md:block md:mb-4" : "mt-4 md:mt-0";
          return (
            <Tag key={pIdx} className={wrapperClass}>
              {paragraph.map((_, lIdx) => {
                const parsed = getHookParsed(pIdx, lIdx);
                const isCurrentLine =
                  pIdx === currentHookParagraph && lIdx === currentHookLine;
                const isCompleted =
                  completedHookLines[pIdx]?.[lIdx] !== undefined;
                ...
                return (
                  <span
                    key={lIdx}
                    className={lIdx > 0 ? "md:block md:-mt-1" : ""}
                  >
  ...
  return (
    <section ref={sectionRef} id="about" className="relative py-16 md:py-24">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(90deg, transparent 0%, hsl(var(--background)) 25%, hsl(var(--background)) 75%, transparent 100%)",
        }}
      />
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6">
        ...
        >
          <motion.div
            ...
            animate={
              typewriterComplete ? { opacity: 1, y: 0 } : { opacity: 0, y: 15 }
            }
            transition={{
              duration: 0.6,
              delay: typewriterComplete ? 0.1 : 0,
              ease: [0.25, 0.46, 0.45, 0.94],
            }}
          >
            <p
              className={`text-base md:text-lg text-muted-foreground leading-relaxed text-center max-w-3xl mx-auto transition-opacity duration-[2500ms] ease-in-out ${textDimmed ? (textRevealed ? "opacity-50" : "opacity-15") : "opacity-100"}`}
            >
              {t.story.why}
            </p>
          </motion.div>
            ...
            {t.story.seeking.map((line, i) => {
              ...
              return (
                <motion.p
                  ...
                  transition={{
                    duration: 0.6,
                    delay: typewriterComplete ? 0.3 + i * 0.2 : 0,
                    ease: [0.25, 0.46, 0.45, 0.94],
                  }}
          ...
          <motion.div
            ...
            transition={{
              duration: 0.6,
              delay: typewriterComplete ? 0.9 : 0,
              ease: [0.25, 0.46, 0.45, 0.94],
            }}
...
type ProjectCategory =
  keyof (typeof translations)["en"]["projects"]["categories"];
...
const PROJECTS: Project[] = [
  {
    ...
    category: "Web App",
    src: onmatutu,
    tags: ["Claude Code", "Cowork", "Remotion"],
    demo: "https://onmatutu.com",
  },
  {
    key: "SkyJoy",
    title: "SkyJoy",
...
const TECH_CATEGORIES: TechCategory[] = [
  {
    ...
    items: [
      ...
      { src: dart, title: "Dart" },
      { src: bloc, title: "BLoC" },
      { src: riverpod, title: "Riverpod" },
      { src: firebase, title: "Firebase" },
      { src: javaAndroid, title: "Java Android" },
      { src: kotlinIcon, title: "Kotlin" },
      { src: providerIcon, title: "Provider" },
    ],
  },
  {
    key: "Frontend",
  ...
  },
  {
    ...
    items: [
      ...
      { src: reactquery, title: "React Query" },
      { src: github, title: "GitHub" },
      { src: formilk, title: "Formik" },
      { src: jenkinsIcon, title: "Jenkins" },
      { src: fastlaneIcon, title: "Fastlane" },
      { src: mqttIcon, title: "MQTT" },
    ],
  },
  {
    key: "AI Tools",
...
function App() {
  ...
  const hydrated = useHydrated();
  const { displayText: roleText, roleIndex } = useTypewriterRotation(
    t.greetingRoles,
  );
  ...
  return (
    <main className="min-h-screen bg-background bg-[length:24px_24px] [background-image:radial-gradient(circle,hsl(var(--dot-grid))_1px,transparent_1px)]">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
            ...
            >
              ...
              <h1 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight mb-4 leading-tight">
                {t.greeting}{" "}
                <span className="text-gradient-theme">{t.name}</span>
                <br />
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-3">
                <a
                  href="#projects"
                  className="flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-theme text-white border border-transparent hover:brightness-110 hover:shadow-xl hover:shadow-primary/30 active:brightness-95 transition-all duration-200 text-sm font-medium shadow-lg shadow-primary/25"
                >
          ...
          <AnimatedSection delay={0.05}>
            <div className="grid grid-cols-3 gap-3 mb-12 max-w-2xl">
              {t.stats.map((s) => (
                <div
                  key={s.label}
                  className="p-4 sm:p-6 rounded-2xl bg-card border border-border text-center"
                >
                  <p className="font-display text-3xl md:text-4xl font-bold text-gradient-theme">
                    {s.value}
                  </p>
      ...
      <section
        ...
        style={{
          contentVisibility: "auto",
          containIntrinsicSize: "auto 2500px",
        }}
      >
        <div className="max-w-5xl mx-auto px-6">
          <AnimatedSection>
            ...
            {PROJECTS.map((p, i) => (
              <AnimatedSection key={p.key} delay={0.05 + (i % 3) * 0.05}>
                <div className="h-full flex flex-col rounded-2xl bg-card border border-border hover:border-primary/30 transition-colors duration-200 overflow-hidden group">
                  ...
                  <div
                    className={`bg-slate-100 overflow-hidden ${p.padded ? "p-4" : ""}`}
                  >
                    <img
                      src={p.src}
                      alt={p.title}
